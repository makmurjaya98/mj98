...
export interface StockHierarchyUser {
  id: number;
  fullName: string;
  username: string;
  role: string;
...
export interface StockHierarchyResponse {
  mitraCabang: StockHierarchyUser[];
  cabang: StockHierarchyUser[];
  links: StockHierarchyUser[];
}

// Retrieves hierarchy data for stock management.
export const getStockHierarchy = api<void, StockHierarchyResponse>(
  { expose: true, method: "GET", path: "/voucher/stock-hierarchy" },
  async () => {
    // Get all Mitra Cabang users
    const mitraCabangUsers: StockHierarchyUser[] = [];
    ...
    const cabangUsers: StockHierarchyUser[] = [];
    ...
    const linkUsers: StockHierarchyUser[] = [];
