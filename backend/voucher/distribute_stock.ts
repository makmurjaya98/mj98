...
// Distributes voucher stock to a specific Link through the hierarchy.
export const distributeStock = api<DistributeStockRequest, DistributeStockResponse>(
  { expose: true, method: "POST", path: "/voucher/distribute-stock" },
  async (req) => {
    ...
    if (existingStock) {
      ...
      await authDB.exec`
        ...
        SET amount = amount + ${req.amount}, updated_at = NOW()
    ...
    return {
      success: true,
      message: `Successfully distributed ${req.amount} ${req.voucherType} vouchers to the selected Link`,
    };
  }
);
